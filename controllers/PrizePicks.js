...
async function getProjections(req, res) {
  ...
  let LINK = 'https://api.prizepicks.com/projections?league_id=7';
    ...
    // Save filtered projections to MongoDB
    await Projection.insertMany(filteredProjections);
...
async function getAllPrizepicksData(req, res) {
  ...
  let LINK = 'https://api.prizepicks.com/projections?league_id=7';
    ...
    if (Array.isArray(jsonData)) {
      ...
      // Modify the filteredData to extract 'league' data as a string
      filteredData.forEach(item => {
        if (item.relationships && item.relationships.league && item.relationships.league.data) {
          item.relationships.league.data = item.relationships.league.data.id;
        }
      });
      ...
      await Player.insertMany(filteredData);
